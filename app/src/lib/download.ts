...
const downloadPdf = (chat: Chat, messages: Message[], filename: string) => {
    ...
    doc.setFontSize(10);
    doc.text(`Date: ${new Date(chat._creationTime).toLocaleString()}`, margin, yPosition);
    yPosition += 10;
    ...
    messages.forEach((message) => {
        ...
        const content = contentStartIndex !== -1 ? fullMessage.substring(contentStartIndex + 1).trim() : fullMessage;

...
const TableHead = ({ className, ...properties }: React.ComponentProps<"th">) => (
    <th
        className={cn("text-muted-foreground h-12 px-3 text-left align-middle font-medium has-[role=checkbox]:w-px [&:has([role=checkbox])]:pr-0", className)}
...
const TableCell = ({ className, ...properties }: React.ComponentProps<"td">) => (
    <td className={cn("p-3 align-middle [&:has([role=checkbox])]:pr-0", className)} data-slot="table-cell" {...properties} />
);

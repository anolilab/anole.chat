...
const SiteHeader: FC<PropsWithChildren<{ menu?: ReactNode; subMenuWidth?: number; title?: string }>> = ({ children, menu, subMenuWidth = 128, title }) => (
    <>
        <header className="relative flex h-(--header-height) shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-(--header-height) dark:text-white">
            <div className="relative z-20 ml-3 flex w-full items-center gap-1 truncate pt-2 lg:gap-2">
            ...
            <div
                className="absolute -top-2 -right-2.5 z-10 h-14.5 max-sm:hidden"
                style={{

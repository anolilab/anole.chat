...
export const settingsViews = ["SETTINGS", "SECURITY", "API_KEYS", "ORGANIZATION", "ORGANIZATIONS", "MEMBERS"] as const;
...
export function SettingsCards({ className, classNames, localization, pathname, view }: SettingsCardsProps) {
    useAuthenticate();

    const { apiKey, basePath, localization: contextLocalization, organization, settings, viewPaths, Link } = useContext(AuthUIContext);

    localization = { ...contextLocalization, ...localization };

    // Determine view from pathname if provided
    const path = pathname?.split("/").pop();
    view = view || (getAuthViewByPath(viewPaths, path) as SettingsView) || "SETTINGS";
    ...
    return (
        <div className={cn("flex w-full grow flex-col gap-4 md:flex-row md:gap-12", className, classNames?.base)}>
            <div className="flex justify-between gap-2 md:hidden">
                <Label className="text-base font-semibold">{currentItem?.label}</Label>
                    ...
                    <DrawerContent className={cn(classNames?.drawer?.content)}>
                        ...
                        <div className="flex flex-col px-4 pb-4">
                            {currentNavigationGroup.map((item) => (
                                <Link key={item.view} href={`${settings?.basePath || basePath}/${viewPaths[item.view]}`}>
                                    <Button
                                        size="lg"
                                        className={cn(
            ...
            {view === "SETTINGS" && <AccountSettingsCards classNames={classNames} localization={localization} />}

            {view === "SECURITY" && <SecuritySettingsCards classNames={classNames} localization={localization} />}
            ...
            {view === "ORGANIZATION" && organization && <OrganizationSettingsCards classNames={classNames} localization={localization} />}
            ...
            {view === "MEMBERS" && organization && (
                <div className={cn("flex w-full flex-col gap-4 md:gap-6", classNames?.cards)}>
                    <OrganizationMembersCard classNames={classNames?.card} localization={localization} />

                    <OrganizationInvitationsCard classNames={classNames?.card} localization={localization} />
                </div>
            )}
        </div>
    );

...
export function arrangeNodes(nodes: UINode[], edges: Edge[]): ArrangeNodesResult {
    ...
    const queue = [{ level: 0, nodeId: inputNode.id }];
    const visited = new Set<string>();

    while (queue.length > 0) {
        const { level, nodeId } = queue.shift()!;
        ...
        children.forEach((childId) => {
            if (!visited.has(childId)) {
                queue.push({ level: level + 1, nodeId: childId });
            }
        });
    }
    ...
    for (let level = 0; level <= maxLevel; level++) {
        if (level === 0)
            continue; // Input node already placed

        const levelNodes = [...levels.entries()].filter(([_, nodeLevel]) => nodeLevel === level).map(([nodeId, _]) => nodeId);
        ...
        const sortedParentGroups = [...parentGroups.entries()].sort(([keyA], [keyB]) => {
            ...
            const getGroupParentAvgY = (parents: string[]) => {
                ...
                const parentYs = parents.map((parentId) => nodePositions.get(parentId)?.y).filter((y) => y !== undefined) as number[];
        ...
        for (const [parentKey, groupNodes] of sortedParentGroups) {
            ...
            if (parents.length === 1) {
                ...
                if (groupNodes.length === 1) {
                    ...
                    const totalRequiredHeight = groupNodes.reduce((sum, nodeToPlace) => sum + nodeToPlace.height + NODE_PADDING, -NODE_PADDING); // Remove last padding
                    ...
                    const adjustedStartY = findNonOverlappingY(startY + firstNodeHeight / 2, firstNodeHeight, placedNodesInLevel);
 ...
 */
function findNonOverlappingY(preferredY: number, nodeHeight: number, placedNodes: PlacedNode[]): number {
    ...
    while (attempts < maxAttempts) {
        ...
        const hasOverlap = placedNodes.some((placed) => !(bottomBound <= placed.topBound || topBound >= placed.bottomBound));
        ...
        const conflictingNodes = placedNodes.filter((placed) => !(bottomBound <= placed.topBound || topBound >= placed.bottomBound));

...
describe("addEdgeBranchLabel", () => {
    ...
    const createNode = (id: string, kind: NodeKind, name: string = `Node ${id}`): DBNode => {
    ...
    const createEdge = (id: string, source: string, target: string, sourceHandle?: string, label?: string): DBEdge => {
        return {
            createdAt: new Date(),
            id,
            source,
    ...
    it("should add B0 label to single linear path", () => {
        ...
        const edges: DBEdge[] = [createEdge("e1", "start", "llm1"), createEdge("e2", "llm1", "end")];

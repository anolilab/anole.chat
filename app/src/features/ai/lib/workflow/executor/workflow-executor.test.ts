...
import { BooleanConditionOperator, StringConditionOperator } from "../condition";
...
vi.mock("../shared.workflow", () => {
    return {
        convertDBNodeToUINode: vi.fn().mockImplementation((databaseNode) => {
            return {
                data: {
                    id: databaseNode.id,
                    kind: databaseNode.kind,
                    name: databaseNode.name,
                    ...databaseNode.nodeConfig,
                },
                id: databaseNode.id,
                position: { x: 0, y: 0 },
                type: "default",
...
describe("createWorkflowExecutor", () => {
    ...
    const createNode = (id: string, kind: NodeKind | "NOOP", name: string = `Node ${id}`, additionalConfig: Record<string, any> = {}): DBNode => {
        return {
            createdAt: new Date(),
            id,
            kind: kind as string,
    ...
    const createEdge = (id: string, source: string, target: string, sourceHandle?: string): DBEdge => {
        return {
            createdAt: new Date(),
            id,
            source,
    ...
    it("1. should execute a simple linear workflow: start -> noop -> end", async () => {
        ...
        const edges: DBEdge[] = [createEdge("e1", "start", "noop"), createEdge("e2", "noop", "end")];

        const executor = createWorkflowExecutor({ edges, nodes });
    ...
    it("2. should handle REAL conditional branching with actual condition evaluation", async () => {
        ...
        // Test TRUE path
        const executor1 = createWorkflowExecutor({ edges, nodes });
        ...
        // Test FALSE path
        const executor2 = createWorkflowExecutor({ edges, nodes });
    ...
    it("4. should handle REAL parallel execution with proper synchronization", async () => {
        ...
        executor.subscribe((event: any) => {
            if (event.eventType === "NODE_START") {
                ...
                nodeExecutionCounts[nodeName] = (nodeExecutionCounts[nodeName] || 0) + 1;
    ...
    it("6. should properly handle workflow execution events and state", async () => {
        ...
        const edges: DBEdge[] = [createEdge("e1", "start", "middle"), createEdge("e2", "middle", "end")];
        ...
        const workflowStartEvent = events.find((e) => e.eventType === "WORKFLOW_START");

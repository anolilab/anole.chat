...
const CONNET_TIMEOUT = IS_VERCEL_ENV ? 15_000 : 120_000;
...
export class MCPClient {
    ...
    getInfo(): MCPServerInfo {
        return {
            ...
            error: this.error,
            name: this.name,
            status: this.locker.isLocked ? "loading" : this.isConnected ? "connected" : "disconnected",
        ...
        try {
            ...
            if (isMaybeStdioConfig(this.serverConfig)) {
                ...
                try {
                    ...
                    this.log.warn("Streamable HTTP connection failed, falling back to SSE transport");
            ...
            this.log.info(`Connected to MCP server in ${((Date.now() - startedAt) / 1000).toFixed(2)}s`);
            ...
            this.tools = toolResponse.tools.reduce((previous, _tool) => {
                ...
                previous[_tool.name] = tool({
                    ...
                    execute: (parameters_, options: ToolExecutionOptions) => {
                        options?.abortSignal?.throwIfAborted();

                        return this.callTool(_tool.name, parameters_);
                    },
                    parameters,
                });
    ...
    async disconnect() {
        ...
        void client?.close().catch((error) => this.log.error(error));
    }
 ...
 */
export const createMCPClient = (name: string, serverConfig: MCPServerConfig, options: ClientOptions = {}): MCPClient =>
    new MCPClient(name, serverConfig, options);

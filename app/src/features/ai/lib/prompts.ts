...
import type { McpServerCustomizationsPrompt, MCPToolInfo } from "@/types/mcp";
...
export const buildUserSystemPrompt = (user?: User, userPreferences?: UserPreferences) => {
    let prompt = `
...
export const buildSpeechSystemPrompt = (user: User, userPreferences?: UserPreferences) => {
    let prompt = `
...
export const buildProjectInstructionsSystemPrompt = (instructions?: Project["instructions"] | null) => {
...
export const buildMcpServerCustomizationsSystemPrompt = (instructions: Record<string, McpServerCustomizationsPrompt>) => {
    const prompt = Object.values(instructions).reduce((accumulator, v) => {
        if (!v.prompt && Object.keys(v.tools ?? {}).length === 0)
            return accumulator;
...
${
    v.tools
        ? Object.entries(v.tools)
            .map(([toolName, toolPrompt]) => `- **${createMCPToolId(v.name, toolName)}**: ${toolPrompt}`)
            .join("\n")
        : ""
}
</${v.name}>
...
export const generateExampleToolSchemaPrompt = (options: { prompt?: string; toolInfo: MCPToolInfo }) => `\n

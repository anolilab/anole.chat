...
export function createOpenAICompatibleModels(config: OpenAICompatibleProvider[]) {
    ...
    try {
        config.forEach(({ apiKey, baseUrl, models, provider }) => {
            ...
            models.forEach(({ apiName, supportsTools, uiName }) => {
...
const OpenAICompatibleModelSchema = z
    .object({
        ...
        supportsTools: z.boolean().describe("Indicates if the model supports external tools/function calling for multi-cloud platform (MCP) servers."),
...
export const OpenAICompatibleProviderSchema = z
    .object({
        ...
        apiKey: z
            .string()
            .describe("The name of the environment variable (e.g., 'OPENAI_API_KEY') for the provider's API key. This key should be stored in a .env file."),
        ...
        models: z.array(OpenAICompatibleModelSchema).describe("A list of AI models offered by this provider."),

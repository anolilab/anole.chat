...
export const PREAUTH_createStripeCustomer = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        const customer = await stripe.customers.create({ email: user.email, name: user.username }).catch((err) => console.error(err));
...
export const createSubscriptionCheckout = action({
    ...
    handler: async (ctx, args): Promise<string | undefined> => {
        ...
        const { currentSubscription, newPlan } = await ctx.runQuery(internal.stripe.getCurrentUserSubscription, { planId: args.planId });
...
export const cancelCurrentUserSubscriptions = internalAction({
    ...
    handler: async (ctx) => {
        ...
        const subscriptions = (await stripe.subscriptions.list({ customer: user.customerId })).data.map((sub) => sub.items);

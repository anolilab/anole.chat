...
import schema, { CURRENCIES, Currency, Interval, INTERVALS, PlanKey, PLANS } from "@cvx/schema";
...
export default internalAction(async (ctx) => {
    ...
    const seededProducts = await asyncMap(seedProducts, async (product) => {
        ...
        const getPrice = (currency: Currency, interval: Interval) => {
            const price = stripePrices.find((price) => price.currency === currency && price.recurring?.interval === interval);
            if (!price) {
    ...
    await stripe.billingPortal.configurations.create({
        ...
        features: {
            ...
            subscription_update: {
                ...
                products: seededProducts.filter(({ key }) => key !== PLANS.FREE).map(({ product, prices }) => ({ product, prices })),
            },
        },
    });
